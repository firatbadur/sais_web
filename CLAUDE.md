...
│   ├── models.py          # SaisCabinet, EnvisoftChannel, SystemSwitch, SimOutboxEntry
│   ├── sim_outbox.py      # Bakanlık gönderim kuyruğu motoru (sıkı FIFO drenaj + backoff + kilit)
│   ├── sim_errors.py      # SendData sonucu → kabul / geçici hata / kalıcı ret (saf fonksiyon)
│   ├── admin.py
│   ├── apps.py
│   ├── management/commands/seed_sais_data.py  # Atıksu StationType + ministry_sample + Envisoft eşlemeleri
│   ├── management/commands/sim_outbox_status.py     # kuyruk teşhisi (salt okuma)
│   ├── management/commands/drain_sim_outbox_once.py # kuyruğu senkron boşalt (debug)
│   ├── management/commands/prune_sim_outbox.py      # kuyruk retention
│   └── migrations/
...
- **Ölü soket (broken pipe) savunması — idle-aware pool + cycle içi retry:** Pool **worker process'e özel** olduğundan prefork'ta (`--concurrency=4`) aynı PLC'ye **worker sayısı kadar socket** açılır; her socket cycle'ların ancak 1/N'inde kullanılır → aralarda **boşta kalır** → PLC boştaki Modbus TCP oturumunu kapatır → biz ölü sokete yazınca **`BrokenPipeError [Errno 32]`** alırız ve o cycle'ın **TÜM** sensörleri bad olur (SIM'e bozuk dakika gider). Saha kanıtı (Mikrodev PLC): tek anda **3 açık socket** + periyodik broken pipe, tüm sensörler aynı saniyede, %2 hata. İki savunma: **(1)** `get_reader` bayat socket'i (son kullanım > `POOL_MAX_IDLE_SEC`, default 60sn; 0=kapalı) kullanmadan önce kapatıp yeniden açar; **(2)** `poll_connection` hata kategorisi `conn_reset` ise (ölü socket) pool'u invalidate edip cycle'ı **bir kez** tekrarlar → veri kaybı olmaz. **Timeout'ta retry YAPILMAZ** (taze socket cevapsız cihazı konuşturmaz, yalnız bloklamayı ikiye katlar) — ayrım `classify_comm_error` ile yapılır. Okuma/persist ayrımı için bkz. `scada_io.tasks._read_cycle`. **Eşzamanlı soketi kesen üç savunma (v1.5.2, Van Erciş / Mikrodev PLC vakası)** — aşağıdaki maddeye bak. Tam izolasyon (polling'i ayrı kuyruk + `--concurrency=1` worker'a almak) hâlâ yapılmadı; compose değişikliği gerektirir.
- **Aynı bağlantıya tek cycle + tek oturum (poll kilidi / `single_session` / `expires`) — saha vakası: Van Erciş, Mikrodev PLC, %63 hata:** Modbus Poll tek soketle kesintisiz okurken bizim yazılım sürekli `ConnectionResetError [Errno 104]` + `BrokenPipeError [Errno 32]` alıyordu; tüm sensörler aynı saniyede birlikte düşüyordu (bağlantı seviyesi). Kök neden **cihaza aynı anda birden fazla TCP oturumu açmamız**: `connection_pool` worker PROCESS'ine özel (prefork `--concurrency=4` → 4 soket) ve `last_polled_at` cycle'ın BAŞINDA damgalandığı için bir cycle `poll_interval_sec`'ten uzun sürerse (timeout × retry bloklaması) `dispatch_polls` aynı bağlantıyı bitmeden yeniden kuyruğa atıyordu → kuyruk birikiyor, eşzamanlı cycle'lar açılıyor, küçük PLC eski oturumu RST ile düşürüyordu. **Timeout'u büyütmek durumu KÖTÜLEŞTİRİR** (cycle uzar → örtüşme artar). Üç katman: **(1)** `poll_connection` bağlantı başına **dağıtık kilit** alır (`cache.add` = Redis SETNX, `scada:poll:{id}`, TTL 150 sn, `finally` ile bırakılır) — aynı anda tek cycle; `dispatch_polls` kilitliyi hiç kuyruğa atmaz. **(2)** enqueue `apply_async(expires=max(30, interval*2))` ile yapılır → kuyrukta bayatlayan poll **çalışmadan düşer** (eskiden birikmiş kuyruk, operatör Sistem Kontrol'den polling'i kapattıktan dakikalar sonra bile cihaza bağlanmaya devam ediyordu — "kapattım ama Son Poll 5 sn önce"); ayrıca `poll_connection` başında `SystemSwitch.polling_enabled` **yeniden** kontrol edilir (eskiden yalnız dispatch'te bakılıyordu) ve Sistem Kontrol'de anahtar kapatılınca açık soketler otomatik `broadcast_close_pools` ile kapatılır. **(3)** `Connection.single_session` (yeni alan, varsayılan **kapalı**) — cihaz tek oturum kabul ediyorsa okuma turu biter bitmez soket kapatılır; kilitle birlikte **her an en fazla bir açık oturum** garantisi verir (Modbus Poll davranışı). Ayrıca `_read_cycle` soket ölünce (`conn_reset`) **kalan sensörleri aynı ölü sokete sormaz** — pymodbus kırık soketi kendi kapatmaz (`client.socket` set kalır, `connect()` True döner) → eskiden cycle'ın geri kalanı sensör başına aynı hatayı ve boşuna beklemeyi üretiyordu; artık hepsi hatayla işaretlenip taze soketle tek seferlik retry'a geçilir. **Mevcut sahada biriken kuyruk bir kez elle temizlenmeli** (`celery -A sais_web purge -f`): `expires` yalnız yeni yayınlanan mesajlara işlenir. **Hangi savunmanın doğru olduğunu ölçerek bulmak için** `diagnose_plc_socket --connection N`: bağlantı ölçeği (tur başına istek, en kötü tur süresi) + cihaza **gerçekten** açık TCP oturum sayısı (`scada_io/netdiag.py` `/proc/net/tcp`; pool worker belleğinde olduğundan `scada_pool_stats` control command'ı ile worker'lara broadcast edilir) + iki soak testi: `persistent` (tek kalıcı soket = Modbus Poll deseni) vs `reconnect` (her turda aç/kapa = `single_session` deseni). Kritik ayrım: **persistent temiz + reconnect hatalı** ise cihaz kapatılan oturumu geç serbest bırakıyordur → `single_session` YANLIŞ çözümdür, polling'i tek process'e almak gerekir; **ikisi de temiz** ise `single_session` yeterlidir; **persistent bile hatalı** ise sorun eşzamanlı oturum değildir (ağ yolu/cihaz).
...
- **Sensör decode:** `scada_io.decoders.decode_registers` 12 data_type (int16/uint16/int32/uint32/int64/uint64/float32/float64/bool/bit/string/raw) × byte_order × word_order kombinasyonlarını destekler. **Endian semantiği:** `byte_order` = register İÇİ byte sırası (`little` = byte swap; tek-register tiplerde de uygulanır), `word_order` = register sırası. float32 wire formatı ↔ ayar: ABCD=`big/big`, CDAB=`big/little`, BADC=`little/big`, DCBA=`little/little` (Modbus Poll gösterim adlarıyla sırasıyla Big-endian / Little-endian byte swap / Big-endian byte swap / Little-endian). Eskiden byte-swap'lı formatlar (BADC/DCBA) çözülemiyordu (pack/unpack aynı endian'ı kullandığından `little/little`≡`big/big` davranıyordu); default `big` olduğu için mevcut saha konfigürasyonları etkilenmez. `Sensor.scale * raw + Sensor.offset` mühendislik dönüşümü uygulanır; `Sensor.decimals` verilmişse `round(value, decimals)` ile float yuvarlaması. Bit-okuma: register içindeki `bit_position`. ASCII: `ascii_request` gönder + `ascii_response_regex` ile değer çıkar.
...
- **Aggregate tabloları:** `ReadingFiveMin` / `ReadingFifteenMin` / `ReadingHourly` / `ReadingDaily` — sensör başına bucket avg/min/max/count/bad_count (hepsi `ReadingAggregateBase` alt sınıfı). Dashboard'lar bunları sorgulasın, raw `Reading`'i taramasın. `aggregate_readings` komutu (`--bucket=5m|15m|hour|day`) veya `api.tasks.aggregate_readings_*` Celery task'larıyla doldurulur. **5m** bucket numune senaryosu motorunun poll-sıklığından bağımsız kısa-pencere ortalaması için her dakika hesaplanır. **Geçerli-veri filtresi (tüm bucket'lar):** avg/min/max yalnız `quality ∉ {bad, uncertain}` okumalardan hesaplanır (`aggregate_readings.EXCLUDED_QUALITIES`) — yıkama/bakım overlay'i `uncertain`, geçersiz/alarm `bad` yazdığından (bkz. `persist_reading`) bu satırlar ortalamayı çarpıtmaz (yıkamada sensör suyla temas etmiyor → numune senaryosu ortalaması sahte yükselmesin). `count`/`bad_count` yine tüm satırları sayar; bucket'ta hiç geçerli okuma yoksa avg/min/max `None`.
...
- **Bakanlık SİM gönderim kuyruğu (store-and-forward outbox) — SAIS-özel → `sais_domain/`:** Dakikalık ölçüm artık `SendData` ile **doğrudan gönderilmez**; önce `SimOutboxEntry` (`sais_sim_outbox`) tablosuna yazılır, gönderimi ayrı bir **drenaj** task'ı yapar. Sebep (saha): Bakanlık servisi yanıt vermediğinde o dakikanın payload'ı hiçbir yere yazılmadan kayboluyordu; tek onarım yolu 6 saatlik `resend_missing_data` idi. **Sıkı FIFO / baş tıkanması:** [sais_domain/sim_outbox.py](sais_domain/sim_outbox.py) `drain_cabinet` bir şeritte en eski bekleyen kayıttan başlar; baştaki kayıt **geçici** hata alırsa (timeout/bağlantı/5xx/auth/JSON-değil) döngü kırılır — sonraki dakikalar Bakanlık o veriyi kabul edene kadar **gönderilmez** (dakika sırası korunur). Kilitlenmeye karşı **dört kaçış valfi**: (1) kalıcı ret (HTTP 4xx / zarf `result:false`) `SAIS_SIM_OUTBOX_MAX_ATTEMPTS`=10 denemeden sonra `failed` + kuyruk ilerler + alarm; (2) **zehirli kuyruk** — peş peşe `POISON_STREAK`=3 kayıt aynı nedenle reddedilirse (sistematik hata) sonraki retler ilk denemede `failed` olur, yoksa dakikada 1 gelen kuyruk 10 deneme × backoff'a asla yetişemezdi; (3) **kabul penceresini** aşan kayıt `expired`; (4) panelden "Atla" (rol=1). Şeritler: canlı (`priority=0`) dakikalar backfill'den (`10`) önce gider, canlı şerit tıkalıysa backfill'e hiç geçilmez. Kuyruk **kabin bazında izole** (drenaj task'ı + Redis kilidi kabin başına). **Payload dondurulur** — aksi halde biriken kuyruğu boşaltan drenaj *güncel* değerleri *eski* dakika damgasıyla gönderirdi. Atomik `pending→sending` sahiplenme + `claimed_at` kiralaması (worker SIGKILL yerse `reap_stuck_entries` kaydı geri alır; aksi halde kısmi unique yüzünden o dakika bir daha kuyruğa alınamazdı). Backoff `min(30·2^(n-1), 600)` sn ±%10 jitter. Motor `sim_outbox.py`, Celery sarmalayıcıları `sais_domain/tasks.py` (`dispatch_sim_outbox` / `drain_sim_outbox`), hata sınıflandırma [sais_domain/sim_errors.py](sais_domain/sim_errors.py) (saf fonksiyon, `comm_errors.py` deseni). **Dashboard:** Sürekli İzleme Merkezi → **Veri Gönderim Kuyruğu** (`/dashboard/admin-pages/sim-outbox/`, operatör+admin) — kabin özet kartları + filtreli DataTables + "Şimdi Gönder"/"Atla"/"Yeniden Kuyruğa Al"/"İletilemeyenleri Temizle"; Sistem Kontrol'de özet kart (10 sn poll). Debug: `sim_outbox_status`, `drain_sim_outbox_once`. Retention: `prune_sim_outbox` (7 gün iletilmiş / 30 gün ölü; bekleyen ASLA silinmez).

- **Bakanlık'a gönderilecek kolon seçimi (`SimChannel`) — SAIS-özel → `sais_domain/`:** Bakanlık
  `SendData`'yı **payload bütünü** düzeyinde doğrular: istasyon tanımında olmayan **tek** bir kolon
  o dakikanın tamamını düşürür (`{"result":false,"message":"Tanımsız Kolonlara veri girilemez
  (Sicaklik)"}`). Üstelik bu **kalıcı** rettir → kuyrukta poison-streak ile dakikalar `failed` olur.
  Saha örneği (Marbin/Tekirdağ): tesiste sıcaklık ölçülüyor ama Bakanlık o istasyon için yalnız
  `Debi`/`Debi_N`/`GirisDebi`/`GirisDebi_N` tanımlamış → tüm iletim kilitlenmişti. Çözüm: hangi
  parametrenin iletileceği `sais_domain.models.SimChannel` (kabin × parametre) ile seçilir;
  `build_sim_payload` **ve** `build_sim_payloads_for_times` (backfill — aksi halde eksik veri
  servisi elenen kolonu geri koyardı) yalnız seçilileri yazar. **Geriye uyumlu:** kabin için hiç
  kayıt yoksa davranış değişmez (hepsi gider) — `SimStatusPolicy.configured` ile aynı yaklaşım;
  `enabled_parameter_names()` bu yüzden `None` (yapılandırılmamış) ile boş küme (hiçbiri) ayrımını
  korur. **UI:** Sürekli İzleme Merkezi → **Gönderilecek Veriler**
  (`/dashboard/admin-pages/sim-channels/`, operatör+admin) — kolon listesi + "Bakanlıktan
  Senkronize Et" (`GetChannelInformation` ile karşı tarafta gerçekten tanımlı kolonları çeker,
  kullanıcı tahmin etmesin) + tanımsız kolon işaretliyken kırmızı uyarı ve kaydetmeden önce onay.
  İlk senkronizasyonda kayıt oluşurken `is_enabled = Bakanlık'ta tanımlı` atanır (ret sebebi
  kendiliğinden dışarıda kalır); sonraki senkronizasyonlar kullanıcı seçimini **ezmez**, yalnız
  tanım bilgisini tazeler. Testler: [sais_domain/tests.py](sais_domain/tests.py)
  `SimChannelSelectionTests` (4).
- **TUZAK — Bakanlık reddi eskiden BAŞARI sayılıyordu (bulundu + düzeltildi):** `SaisSimClient.send_data` yanıtı `_unwrap(..., allow_non_dict=True)` ile açıyordu; `_unwrap` zarfta `objects` anahtarı varsa **`payload["objects"]`** döndürür. Bakanlık'ın tipik ret zarfı `{"result": false, "message": "...", "objects": null}` olduğu için `send_data` **`None`** dönüyor, çağırandaki `isinstance(result, dict) and "result" in result` kontrolü `False` kalıyor ve `accepted = True` sayılıyordu → `SystemSwitch.mark_sim_success()` yazılıyor, panelde **"SİM'e Son İletim" yeşil görünürken Bakanlık veriyi almamış** oluyordu (aynı hata `resend_missing_data`'nın `resent` sayacını da şişiriyordu). `send_data` artık `_envelope()` kullanır (ham zarf; `send_calibration`/`send_host_changed` ile aynı sözleşme) ve kabul/ret ayrımını `sim_errors.classify_send_result` yapar; `mark_sim_success` YALNIZ gerçek kabulde atılır. **Aynı yutma `send_diagnostic*` ve `sample_request_*` uçlarında hâlâ var** (kapsam dışı — bkz. bilinen sınırlamalar).

- **Bakanlık veri kabul penceresi ayarlanabilir (sabit 48 saat DEĞİL):** Bakanlık varsayılan olarak son **48 saatlik** veriyi kabul eder, ama bakım/özel durumlarda bu pencereyi geçici olarak genişletebiliyor. Bu yüzden pencere kod sabiti değil `SystemSwitch.sim_accept_window_hours` alanıdır ve **Sistem Kontrol sayfasından operatör değiştirir** (`save_accept_window` action; 1 saat–30 gün arasına sıkıştırılır). Tek okuma noktası `sim_outbox.accept_window_hours()`; hem kuyruk süre aşımı (`expired`) hem `_parse_missing_dates` filtresi bunu kullanır. `.env`'deki `SAIS_SIM_OUTBOX_MAX_AGE_HOURS` yalnız ilk varsayılanı verir.

- **SİM HTTP timeout + retry (env'e bağlı) — okuma tekrarı bilinçli olarak 0:** Eskiden timeout sabit 50 sn'di ve **hiç** retry yoktu (yalnız 401 → re-login). Artık `BaseHttpClient` bir `urllib3.Retry` + `HTTPAdapter` mount eder; `SAIS_SIM_TIMEOUT`=120, `SAIS_SIM_CONNECT_TIMEOUT`=10, `SAIS_SIM_HTTP_RETRIES`=3, backoff 2.0 (2/4/8 sn, tavan 30). **POST açıkça `allowed_methods`'a eklenir** — urllib3 POST'u idempotent saymaz ve Bakanlık'ın TÜM uçları POST'tur, izin verilmezse retry hiç devreye girmezdi; güvenli çünkü `SendData` `(Stationid, Readtime, Period)` ile idempotenttir. **`SAIS_SIM_HTTP_READ_RETRIES`=0 — DEĞİŞTİRME:** okuma timeout'unu transport'ta tekrarlamak tek çağrıyı `120×4 + backoff ≈ 494 sn`'ye çıkarır, `CELERY_TASK_TIME_LIMIT=300`'ü aşar → worker SIGKILL yer ve `CELERY_TASK_ACKS_LATE=True` yüzünden mesaj yeniden teslim edilir (mükerrer gönderim + askıda kilit). Okuma tekrarı **kuyruk seviyesinde** (`next_attempt_at` backoff'u) yapılır; orada bekleme worker'ı bloklamaz. Ağır sorgu uçları (`GetMissingDates` / `GetDataByBetweenTwoDate`, 180 sn timeout) ayrı **düşük-retry session** kullanır (`SAIS_SIM_LONG_QUERY_RETRIES`=1) — aksi halde kullanıcı-tetikli SIM konsolu 12 dk bekleyebilirdi. ApiLog tüm denemeler için **tek satır** yazar ama `retry_count` alanına gerçek deneme sayısı (`response.raw.retries.history`) düşer.

- **`publish_minute_data` dakikayı SABİTLER + `expires`:** `readtime` artık beat task'ında bir kez hesaplanıp (`replace(second=0, microsecond=0)`) alt task'a parametre geçilir; yıkama `force_status` da tek yerde çözülür (eskiden her kabin task'ı ayrı okuyup aynı bayrağı eşzamanlı temizlemeye çalışıyordu). Eskiden `readtime` alt task'ın **çalıştığı** anda hesaplanıyordu → worker kuyruğu birikince task ait olduğu dakikayı değil koştuğu dakikayı damgalıyor, bir dakika mükerrer yazılırken başkası hiç gönderilmiyordu. **Damgayı sabitlemek `expires` olmadan güvenli değildir** (geç çalışan task eski damgaya *taze* snapshot yazar) → `SAIS_PUBLISH_TASK_EXPIRES_SEC`=45 ile bayat mesaj sessizce düşer. `readtime_iso` verilmezse eski davranışa düşülür — sürüm geçişinde broker'da kalmış mesajlar için zorunlu geri-uyumluluk dalı.

- **SIM kapalıyken de kuyruğa yazılır:** `SystemSwitch.sim_enabled=False` artık yalnız **drenajı** durdurur, üretimi değil (`SAIS_SIM_OUTBOX_ENQUEUE_WHEN_DISABLED=1`). Aksi halde "SIM'i 10 dakikalığına kapatayım" denen sürede geçen dakikalar kalıcı kaybolurdu. Kuyruk kabul penceresiyle sınırlı olduğundan sınırsız büyümez; bakım adımları (`reap`/`expire`) SIM kapalıyken de çalışır ki uzun kapalı kalan sahada açılışta değersizleşmiş binlerce dakika gönderilmeye çalışılmasın. Kuyruk sayfası bu durumda sarı uyarı şeridi gösterir.

- **Eşzamanlı login serileştirildi:** `SaisSimClient.login()` önce `clear_session()` çağırdığı için aynı kabine paralel giden çağıranlar (kuyruk drenajı, senaryo motoru, sistem alarmları, dashboard SIM konsolu) birbirinin ticket'ını geçersiz kılabiliyordu. `clients/auth.py` `acquire_login_lock`/`release_login_lock` (Redis `SETNX`, `SAIS_SIM_LOGIN_LOCK_SEC`=30) ile gerçek login'i tek çağıran yapar; bekleyenler cache'te beliren taze ticket'ı kullanır.
...
- **Parametre Tanımları sayfası (Sensör Ayarları → Parametre Tanımları):** `Parameter` (ölçülen büyüklük tanımı) için sensör sayfasıyla **aynı iskelette** tam CRUD — `/dashboard/sensor-config/parameters/[create|<pk>/edit|<pk>/delete]/`, operatör+admin (`OperatorRequiredMixin`). Liste arama (ad/kod/birim) + tesis filtresi + bağlı sensör sayısı rozetiyle; form `ParameterConfigForm` ([dashboard/forms.py](dashboard/forms.py)) tüm alanları kapsar (kimlik/birim + range + geçerli veri + ölçüm sınırı + kanal eşlemesi collapse'ı) — sihirbaz modalındaki `ParameterQuickForm`'un tam sürümü. `parameter_name` (Bakanlık/Envisoft kanal kodu) form seviyesinde **benzersiz** doğrulanır (model kısıtı yok). **Silme koruması:** `Sensor.parameter` CASCADE olduğundan bağlı sensörü olan parametrenin silinmesi **reddedilir** (onay sayfası bağlı sensörleri listeler); önce sensörler taşınmalı/silinmelidir. Tüm işlemler `log_event(EventType.CONFIG)` ile denetim kaydına düşer.
...
| `sais_domain.tasks.resend_missing_data` | `0 */6 * * *` | Bakanlık `GetMissingDates` → eksik dakikaları `Reading`'den backfill → **gönderim kuyruğuna** yaz (kabin başına `resend_cabinet_missing` fan-out; lisans + `sim_enabled` gate'li) |
| `sais_domain.tasks.dispatch_sim_outbox` | her 30 sn | SİM kuyruğu: takılı kayıtları kurtar (`reap`) + kabul penceresini aşanları `expired` yap + vadesi gelen kabinlere `drain_sim_outbox` gönder |
| `sais_domain.tasks.prune_sim_outbox_task` | `40 3 * * *` | SİM kuyruğu retention (7 gün iletilmiş / 30 gün ölü; bekleyen silinmez) |
...
python manage.py diagnose_plc_socket --connection N    # TCP oturum/soket teşhisi + soak testi
python manage.py sim_outbox_status [--head 10]         # SİM gönderim kuyruğu durumu (salt okuma)
python manage.py drain_sim_outbox_once [--cabinet N]   # kuyruğu senkron boşalt (worker'sız)
python manage.py prune_sim_outbox --dry-run            # kuyruk retention sayımı
```
...
- **`_unwrap` ret-yutma hatası diğer SIM uçlarında sürüyor** — `send_data` `_envelope()`'a taşındı ama
  `send_diagnostic` / `send_diagnostic_with_type` / `sample_request_*` hâlâ `_unwrap(allow_non_dict=True)`
  kullanıyor; Bakanlık `result:false` derse çağıran (senaryo motoru) bunu başarı sayar. Aynı desenle
  düzeltilmeli.
- **Envisoft'un kuyruğu/onarım yolu yok** — SİM store-and-forward kuyruğuna alındı, Envisoft hâlâ
  gönder-unut (HTTP status bile kontrol edilmiyor) ve bir kesintide o dakikalar kalıcı kayboluyor.
- **SİM polling'le aynı Celery kuyruğunu paylaşıyor** — `task_routes` yok, `--concurrency=4`. Bakanlık
  yavaşladığında drenaj bir slotu ~156 sn tutabilir. Kalıcı çözüm `sais_domain.tasks.*`'ı ayrı bir
  `sim` kuyruğuna + kendi worker'ına almak (compose değişikliği gerektirir).
