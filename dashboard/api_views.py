...
DIGITAL_DATA_TYPES = {"bool", "bit"}


@login_required
def home_scada(request):
    """SCADA görünümü için snapshot — analog + dijital ayrımlı, istasyon filtreli.

    Query: ?station_id=<int> opsiyonel; verilmezse tüm aktif istasyonlar.
    Dönüş: {"stations":[...], "selected_station":..., "analog":[...], "digital":[...]}
    """
    stations_qs = Station.objects.filter(active=True).order_by("name")
    stations = [{"id": s.id, "name": s.name} for s in stations_qs]

    selected = request.GET.get("station_id") or ""
    try:
    ...
    if selected_id:
        qs = qs.filter(sensor__connection__station_id=selected_id)
    qs = qs.order_by("sensor__parameter__parameter_name", "sensor_id")

    analog = []
    digital = []
    for latest in qs[:300]:
        sensor = latest.sensor
        if sensor is None:
            continue
        parameter = getattr(sensor, "parameter", None)
        connection = getattr(sensor, "connection", None)
        station = getattr(connection, "station", None) if connection else None
        is_digital = (sensor.data_type or "") in DIGITAL_DATA_TYPES

        item = {
            "sensor_id": sensor.pk,
            "name": (parameter.parameter_name if parameter else None) or f"Sensor-{sensor.pk}",
            "unit": (parameter.unit_txt if parameter else None) or (parameter.unit if parameter else None) or "--",
            "value": latest.value,
            "quality": latest.quality,
            "readtime": latest.readtime.isoformat() if latest.readtime else None,
            "decimals": sensor.decimals,
            "min_range": parameter.min_range if parameter else None,
            "max_range": parameter.max_range if parameter else None,
            "station_name": station.name if station else None,
        }
        if is_digital:
            try:
                bool_value = bool(float(latest.value)) if latest.value not in (None, "") else None
            except (ValueError, TypeError):
                bool_value = None
            item["active"] = bool_value
            digital.append(item)
        else:
            analog.append(item)
