...
#: ``MimicScreen.data`` için üst sınır. Belge her kayıtta ve her görüntüleyici
#: sayfasında (inline ``json_script``) taşındığı için kontrolsüz büyüme sayfayı
#: yavaşlatır/öldürür.
#:
#: **Django'nun ``DATA_UPLOAD_MAX_MEMORY_SIZE``'ının (varsayılan 2.5 MB) ALTINDA
#: tutulmalı** — aksi halde bu kontrole hiç ulaşılmaz ve istek Django tarafından
#: opak bir 400 ile reddedilir (kullanıcı ne olduğunu anlamaz).
...
MIMIC_TOO_BIG_MSG = ("Tasarım verisi çok büyük (üst sınır 2 MB). Nesne sayısını "
                     "azaltın veya tasarımı birkaç mimiğe bölün.")
...
def mimic_screen_save(request):
    ...
    try:
        ...
        # gelmiş olsa da body okunamaz. Django'nun opak 400'ü yerine anlaşılır
        # Türkçe mesaj ver.
